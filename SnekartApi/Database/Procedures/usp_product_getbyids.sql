-- SQL Server had no array parameter type, so the original passed a JSON string unpacked
-- via OPENJSON. Postgres has real arrays, so this takes int[] directly — simpler and faster.
CREATE OR REPLACE FUNCTION usp_product_getbyids(p_ids INT[])
RETURNS TABLE (
